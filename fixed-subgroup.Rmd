...
# temp <- list.files(path='D:\\MPH-Project\\listdatanew\\', pattern="*.rdata")
# 
...
# for (i in 1) {
#    xx[i]<- list.load(paste0("D:\\MPH-Project\\listdatanew\\", temp[[i]], ".rdata"))
# }

# FE_net_age_agd.sc1.1 <- list.load(paste0("D:\\MPH-Project\\listdatanew\\", temp[1], ".rdata"))
# FE_net_age_agd.sc1.2 <- list.load(paste0("D:\\MPH-Project\\listdatanew\\", temp[2], ".rdata"))
# FE_net_age_agd.sc2.1 <- list.load(paste0("D:\\MPH-Project\\listdatanew\\", temp[3], ".rdata"))
# FE_net_age_agd.sc2.2 <- list.load(paste0("D:\\MPH-Project\\listdatanew\\", temp[4], ".rdata"))
# FE_net_age_agd.sc3.1 <- list.load(paste0("D:\\MPH-Project\\listdatanew\\", temp[5], ".rdata"))
# FE_net_age_agd.sc3.2 <- list.load(paste0("D:\\MPH-Project\\listdatanew\\", temp[6], ".rdata"))
# FE_net_age_agd.sc4.1 <- list.load(paste0("D:\\MPH-Project\\listdatanew\\", temp[7], ".rdata"))
# FE_net_age_agd.sc4.2 <- list.load(paste0("D:\\MPH-Project\\listdatanew\\", temp[8], ".rdata"))
# 
# FE_net_org_df <- list.load(paste0("D:\\MPH-Project\\listdatanew\\", temp[9], ".rdata"))
...
## Modify the data to simulate the treatment more effective in younger population
...
sum(!full_ipd$complete)
mean(!full_ipd$complete)

# remove the missing date
...
ggplot(full.crs) +
    ...
    labs(title = "Distribution before simulation")
...
full_ipd.muta$data <- map(full_ipd.muta$data, function(df){
  df %>%
    mutate(lp2 = if_else(age < 47 & trtc %in% c("IXE_Q2W", "IXE_Q4W"), lp + 0.2, lp),
           pasimean = mean(pasi75),
           paismeanquant = mean(pasi75 > pasimean),
           pasi75_sim = if_else(lp2 >= pasimean, 1, 0))
})
...
ggplot(full.crs2) +
    geom_bar(aes(x=sub_age, y=n, fill = cat_pas), stat = "identity") +
    facet_wrap(vars(studyc, trtc)) +
    labs(title = "Distribution after distribution")

full_agd <- full_agd %>%
  mutate(age_mean = ifelse(trtc %in% c("SEC_300", "SEC_150"), age_mean - 5, age_mean))
...
```{r ss}
# type=response, and change in the IXE_Q2W/IXE_Q2W and adapt_delta
...
rlist::list.save(FE_net_org_df, 'D:\\MPH-Project\\listdatanew\\FE_net_org_df.rdata')
rlist::list.save(FE_net_age_agd.sc1.1, 'D:\\MPH-Project\\listdatanew\\FE_net_age_agd.sc1.1.rdata')
rlist::list.save(FE_net_age_agd.sc1.2, 'D:\\MPH-Project\\listdatanew\\FE_net_age_agd.sc1.2.rdata')
rlist::list.save(FE_net_age_agd.sc2.1, 'D:\\MPH-Project\\listdatanew\\FE_net_age_agd.sc2.1.rdata')
rlist::list.save(FE_net_age_agd.sc2.2, 'D:\\MPH-Project\\listdatanew\\FE_net_age_agd.sc2.2.rdata')
rlist::list.save(FE_net_age_agd.sc3.1, 'D:\\MPH-Project\\listdatanew\\FE_net_age_agd.sc3.1.rdata')
rlist::list.save(FE_net_age_agd.sc3.2, 'D:\\MPH-Project\\listdatanew\\FE_net_age_agd.sc3.2.rdata')
rlist::list.save(FE_net_age_agd.sc4.1, 'D:\\MPH-Project\\listdatanew\\FE_net_age_agd.sc4.1.rdata')
rlist::list.save(FE_net_age_agd.sc4.2, 'D:\\MPH-Project\\listdatanew\\FE_net_age_agd_sc4_2.rdata')
...
## set the coefficients together for forest figures
...
coeff.mod.f <- coeff.mod1.f %>% filter(grepl('beta\\[age', parameter)) %>%
  arrange(parameter, scenario) %>%
  mutate("Mean (95% CI)" = paste0(mean, " (", lower, ", ", upper, ")"))

ggplot(coeff.mod.f, aes(x = mean, y = parameter, color = factor(scenario))) +
    geom_point(position = position_dodge(width = 0.4)) +
  geom_errorbarh(aes(xmax = upper, xmin = lower), height=.1, position = position_dodge(width = 0.4)) +
  geom_vline(xintercept=0, color="black", linetype="dashed", alpha=.5) +
  geom_text(aes(x = -0.4, label = `Mean (95% CI)`), position = position_dodge(width = 0.4)) +
  scale_x_continuous(limits = c(-0.55, 0.2))
